...
#### Step 2a 结果（2026-09-26，50 条 / 60 天，四档 criteria + state v1 带上轮摘录）

50/50 成功 0 错误；延迟 median 210ms / p95 305ms（达标；仅空闲后前 2 次调用有 ~0.9-1.3s GPU 暖机尖峰，常驻服务下每小时最多一次，可接受）。

| 指标 | 结果 |
|---|---|
| 四档分布 | balanced 43 / strong 4 / light 3 / **flagship 0** |
| **升档信号** | **0/50——flagship 从未被建议**，升档通道实践中休眠，成本风险≈0 |
| conf≥0.6（live 通道） | 13/50（26%）生效，**全部→balanced**（无 light/无升档）——极保守起步画像 |
| conf≥0.5 | 20/50 → 19 balanced + 1 light |
| state v1（带上轮摘录）验证 | 追问类轮次置信度大幅提升（0.135→0.948 / 0.265→0.949 / 0.316→0.863 / 0.227→0.809）；创作任务翻正为 strong（0.225→strong）——**多轮上下文是路由置信度的最大杠杆** |
| 新阶梯 vs 校准真值（旧 18 条） | 12/18 严格一致（miss 4 条全是 local→light/balanced 粒度漂移，成本影响趋零；2 条 strong 低估仍 <0.6 被阈值兜住） |
