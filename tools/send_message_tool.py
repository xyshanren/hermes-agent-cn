...
_WEINX_TARGET_RE = re.compile(r"^\s*((?:wxid|gh|v\d+|wm|wb)_[A-Za-z0-9_-]+|[A-Za-z0-9._-]+@chatroom|filehelper)\s*$")
_YUANBAO_TARGET_RE = re.compile(r"^\s*((?:group|direct):[^:]+)\s*$")
# Telegram topic targets are numeric IDs (optionally with message thread id).
# Re-added in cn: upstream's TUI v2 split added _TELEGRAM_TOPIC_TARGET_RE which
# the new test_telegram_text_batching tests import transitively via this module.
...
async def _send_to_platform(platform, pconfig, chat_id, message, thread_id=None, media_files=None, force_document=False):
    ...
    if media_files and not message.strip():
        return {
            "error": (
                f"send_message MEDIA delivery is currently only supported for weixin, yuanbao and feishu; "
    ...
    if media_files:
        warning = (
            ...
            "native send_message media delivery is currently only supported for weixin, yuanbao and feishu"
        )
