...
class GatewaySlashCommandsMixin:
    ...
    async def _handle_goal_command(self, event: "MessageEvent") -> str:
        ...
        Subcommands: ``/goal`` / ``/goal status`` / ``/goal show`` /
        ``/goal pause`` / ``/goal resume`` / ``/goal clear``. Any other
        text becomes the new goal. Inline ``field: value`` lines parse
        into a completion contract; ``/goal draft <objective>`` asks the
        goal_judge aux model to draft one.
        ...
        """
        # Lazy imports keep the gateway module import-clean for non-goal
        # surfaces and avoid circular imports during boot.
        ...
        if lower == "show":
            ...
            base = mgr.status_line()
            if contract_block:
                return f"{base}\nCompletion contract:\n{contract_block}"
            return f"{base}\n(Couldn't find a contract — running as a free-form goal.)"

        # /goal draft <objective> → ask the aux model to expand into a
        # structured completion contract, then set it. The aux LLM call
        # is sync; run it off the event loop.
