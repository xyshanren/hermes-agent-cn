...
## v0.15.0+cn.9 (2026-06-18) — 全部 73 KEEP commits 合并完成

> **D1 16/16 + D2 32/32 + D3 25/25 = 61 个 commit 全部合并**（12 EMPTY 跳过），测试修复 8 项。
...
### Phase 4 — D3 大文件合并（25 个）

22 个已在之前 cherry-pick 中自动合并，3 个手工处理：

| Commit | 改动 | 冲突解决 |
|--------|------|---------|
| `dca11b665` | MCP stdio argv passthrough (243 行) | subcommands/mcp.py modify/delete |
| `ed9e8ba09` | dashboard-auth 可插拔密码登录 (387 行) | 5 文件 modify/delete → accept |
| `6717914e0` | WS 拒绝原因说明 (229 行) | 自动合并 |

### 测试修复（8 项）

| # | 修复 | 类型 |
|---|------|------|
| 1 | `hermes_cli/nous_account.py` — 从上游检出 | 缺失文件 import |
| 2 | `hermes_cli/dashboard_auth/public_paths.py` — 从上游检出 | 缺失文件 import |
| 3 | `providers.py` — 添加 bedrock 别名 | 配置缺失 |
| 4 | `agent/curator.py` — 改用 `atomic_json_write()` | 原子写入重构 |
| 5 | `hermes_cli/commands.py` — 添加 `gquota` 命令 | 命令注册缺失 |
| 6 | `locales/zh.yaml` + `zh-hant.yaml` — 补充 i18n 键 | i18n 同步 |
| 7 | `agent/auxiliary_client.py` — `set_runtime_main()` 参数扩展 | API 签名缺失 |
| 8 | `tests/cli/test_branch_command.py` — 断言修正 | 测试 CN 适配 |
