...
Ported back 2026-09-19 with the S12 wiring restore; the swap record now
uses the CAND-080 layer-2 structured format (rule_id = KNOWN_RULES family
name ``cost_aware_fallback`` + ``rule_params`` carrying the chain index
and provider) instead of the legacy formatted label string.
"""
...
class TestSessionThresholdFallbackMode:
    ...
    def test_fallback_mode_activates_next_chain_entry(self, monkeypatch):
        ...
        # CAND-080 layer 2: rule_id is the KNOWN_RULES family name and the
        # activated chain position/provider travel as structured
        # ``rule_params``.  Started at 1 (openai already burned), activated
        # chain[1] → agent._fallback_index is now 2 → chain_index = 1.
        assert routing["rule_id"] == "cost_aware_fallback"
        assert routing["rule_params"] == {"chain_index": 1, "provider": "anthropic"}
