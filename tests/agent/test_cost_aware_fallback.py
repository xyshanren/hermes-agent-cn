...
Ported back 2026-09-19 with the module restore (v0.19.1/v0.20.0 base bump
had dropped the wiring while agent/routing_decision.py survived).
"""
...
class TestCostAwareFallbackConfigFromDict:
    ...
    def test_default_constants_match_config_defaults(self):
        """The defaults baked into hermes_cli.config_defaults.DEFAULT_CONFIG
        and the defaults in cost_aware_fallback.py must stay in sync.  When
        the config default changes, update DEFAULT_PER_REQUEST_MAX_USD /
        ...
        # These constants are what config_defaults.py should mirror:
        assert DEFAULT_PER_REQUEST_MAX_USD == 0.05
        assert DEFAULT_PER_SESSION_MAX_USD == 1.00
        assert DEFAULT_ON_SESSION_EXCEEDED == "warn"
        default_cfg = (DEFAULT_CONFIG.get("agent") or {}).get(
            "cost_aware_fallback"
        ) or {}
        assert default_cfg.get("enabled") is False
        assert default_cfg.get("per_request_max_usd") == DEFAULT_PER_REQUEST_MAX_USD
        assert default_cfg.get("per_session_max_usd") == DEFAULT_PER_SESSION_MAX_USD
        assert default_cfg.get("on_session_exceeded") == DEFAULT_ON_SESSION_EXCEEDED
