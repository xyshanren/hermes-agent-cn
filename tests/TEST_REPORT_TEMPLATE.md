...
> **重要**: 完成测试后，请将实际测试结果填入此文件（不要提交到远程仓库），截图请保存到 `tests/image/` 目录。
...
- 配置成功保存到 `~/.hermes/config.yaml` 和 `~/.hermes/.env`
- 无崩溃或异常报错

**测试结果**: ⬜ 通过 / ❌ 失败  
**备注**: ____________  
**截图**: `tests/image/setup_*.png`
