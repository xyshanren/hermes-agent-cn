...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("save", "保存当前对话", "会话",
               cli_only=True),
    CommandDef("retry", "重试上一条消息（重新发送给 AI）", "会话"),
    CommandDef("undo", "删除最后一轮用户/AI 交换", "会话"),
    CommandDef("title", "设置当前会话标题", "会话",
               args_hint="[名称]"),
    CommandDef("branch", "创建分支会话（探索不同路径）", "会话",
    ...
    CommandDef("approve", "批准待处理危险命令", "会话",
               gateway_only=True, args_hint="[session|always]"),
    CommandDef("deny", "拒绝待处理危险命令", "会话",
               gateway_only=True),
    CommandDef("background", "在后台运行提示词", "会话",
               aliases=("bg",), args_hint="<提示词>"),
    CommandDef("btw", "利用当前上下文临时提问（不使用工具，不保存）", "会话",
               args_hint="<问题>"),
    ...
    CommandDef("verbose", "循环切换工具输出详细程度：off -> new -> all -> verbose", "配置",
               cli_only=True,
               gateway_config_gate="display.tool_progress_command"),
    CommandDef("yolo", "切换 YOLO 模式（跳过所有危险命令确认）", "配置"),
    CommandDef("reasoning", "管理推理强度和显示", "配置",
               args_hint="[级别|show|hide]",
               subcommands=("none", "minimal", "low", "medium", "high", "xhigh", "show", "hide", "on", "off")),
    CommandDef("fast", "切换极速模式 — OpenAI 优先处理 / Anthropic 快速模式", "配置",
               args_hint="[normal|fast|status]",
               subcommands=("normal", "fast", "status", "on", "off")),
    CommandDef("skin", "显示或切换界面主题/皮肤", "配置",
    ...
    CommandDef("toolsets", "列出可用工具集", "工具与技能",
               cli_only=True),
    CommandDef("skills", "搜索、安装、查看或管理技能", "工具与技能", cli_only=True,
               subcommands=("search", "browse", "inspect", "install")),
    CommandDef("cron", "管理定时任务", "工具与技能",
               cli_only=True, args_hint="[子命令]",
               subcommands=("list", "add", "create", "edit", "pause", "resume", "run", "remove")),
    CommandDef("reload", "重新加载 .env 变量到当前会话", "工具与技能"),
    CommandDef("reload-mcp", "从配置重新加载 MCP 服务器", "工具与技能",
               aliases=("reload_mcp",)),
    CommandDef("browser", "将浏览器工具连接到你的 Chrome（CDP）", "工具与技能",
    ...
    CommandDef("insights", "显示用量洞察和分析", "信息",
               args_hint="[天数]"),
    CommandDef("platforms", "显示网关/消息平台状态", "信息",
               cli_only=True, aliases=("gateway",)),
    CommandDef("paste", "检查剪贴板中的图片并附加到输入", "信息",
               cli_only=True),
    CommandDef("image", "附加本地图片文件到下一条输入", "信息",
