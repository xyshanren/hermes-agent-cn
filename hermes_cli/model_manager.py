...
def _get_hermes_home() -> Path:
    ...
    Routes through ``hermes_constants.get_hermes_home()`` so each profile
    gets its own model store; ``HERMES_MODELS_DIR`` still overrides the
    models subdirectory itself.
    """
    try:
